...
final class Controller {
    ...
    private Command[] _commands = {
            ...
            new Command("auto\\s+(\\w+)$", this::doAutomatically),
            new Command("manual\\s+(\\w+)$", this::dobyHand),
            new Command(Move.MOVE_PATTERN.toString(), this::doMakeMove)
    };
    ...
    /** Command "new". */
    private void doNew(Matcher filler) {
        _board.init();
        _winner = null;
    }

    /** Command "quit". */
    private void doQuit(Matcher filler) {
        _playing = false;
    }
    ...
    /** Stuff MATCH. */
    private void doAutomatically(Matcher match) {
        if (match.group(1).equals("black")
                || match.group(1).equals("Black")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        } else if (match.group(1).equals("white")
                || match.group(1).equals("White")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
        }
    }

    /** Stuff MATCH. */
    private void dobyHand(Matcher match) {
        if (match.group(1).equals("black")
                || match.group(1).equals("Black")) {
    ...
    /** Stuff MATCH. */
    private void doMakeMove(Matcher match) {
        Move step = Move.mv(match.group(0));
        if (_board.isLegal(step)) {
            _board.makeMove(step);
            if (board().winner() != null) {
                reportNote(board().winner().toName() + " wins.");
            }
        }
    }

    /** Dump the contents of the board on standard output. */
    private void doDump(Matcher filler) {
